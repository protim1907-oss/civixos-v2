...
const STATE_ABBR = "WI";
// Wisconsin districts are stored zero-padded (WI-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Wisconsin ${ordinal(num)} Congressional District`,
  ...
  // Gov Tony Evers (sitting through Jan 2027), Lt. Gov Sara Rodriguez, AG Josh Kaul (all D).
  const execDefs = [
    { name: "Tony Evers", office: "Governor of Wisconsin", party: "Democrat", website: "https://evers.wi.gov", contact: "https://evers.wi.gov/Pages/Contact-Us.aspx", q: "Tony Evers Governor Wisconsin" },
    { name: "Sara Rodriguez", office: "Lieutenant Governor of Wisconsin", party: "Democrat", website: "https://ltgov.wi.gov", contact: "https://ltgov.wi.gov/Pages/Contact.aspx", q: "Sara Rodriguez Lieutenant Governor Wisconsin" },
    { name: "Josh Kaul", office: "Attorney General of Wisconsin", party: "Democrat", website: "https://www.doj.state.wi.us", contact: "https://www.doj.state.wi.us/contact-us", q: "Josh Kaul Wisconsin Attorney General" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/wi/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Wisconsin State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear WI rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear WI representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert WI representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear WI district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert WI district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Wisconsin: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
