...
const { data: c } = await s.from("outreach_campaigns").select("*").eq("name","MVP prospects — US / Europe / UAE").single();
const names = new Set();
...
const { data: pool } = await s.from("outreach_leads").select("*").contains("tags",["mvp-0909"]).not("email","is",null).order("created_at",{ascending:true}).limit(300);
const next = pool.filter(l=>!done.has(l.id)).slice(0,25);
console.log(`New-list (mvp-0909) unmessaged leads: ${pool.filter(l=>!done.has(l.id)).length}; taking ${next.length}`);
...
const system=[`You write concise, professional B2B cold-outreach emails on behalf of ${org}.`,`What ${org} offers: ${offering}.`,"Rules:","- Be genuine and specific to the recipient and their company; never sound like mass spam.","- 70-130 words. Plain, warm, direct. No hype, no emojis, no ALL-CAPS.","- Honest subject line reflecting the body (CAN-SPAM). No clickbait.","- Lead with a relevant hook, then one clear low-friction CTA.","- Do NOT fabricate facts.","- Do NOT include a signature block, unsubscribe line, or postal address — appended automatically.",'Return ONLY minified JSON: {"subject":"...","body":"..."}'].join("\n");
...
for(const lead of next){
  try{
    ...
    if(L.slice(-3).some(x=>closing.test(x)) || names.has((L[L.length-1]||"").toLowerCase())) bad++;
...
console.log(`MVP new-list batch: generated & APPROVED ${ok} (fail ${fail}, sign-off issues ${bad}). MVP totals: ${JSON.stringify(by)}`);
