...
const STATE_ABBR = "MS";
// Mississippi districts are stored zero-padded (MS-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Mississippi ${ordinal(num)} Congressional District`,
  ...
  const usSenators = stFederal
    ...
    .map((p) => {
      ...
      return representative({
        name: p.name.official_full,
        office: "U.S. Senator, Mississippi",
        level: "senate",
        website: t.url,
  ...
  const execDefs = [
    { name: "Tate Reeves", office: "Governor of Mississippi", party: "Republican", website: "https://governorreeves.ms.gov", contact: "https://governorreeves.ms.gov/contact/", q: "Tate Reeves Governor Mississippi" },
    { name: "Delbert Hosemann", office: "Lieutenant Governor of Mississippi", party: "Republican", website: "https://ltgovhosemann.ms.gov", contact: "https://ltgovhosemann.ms.gov/contact/", q: "Delbert Hosemann Lieutenant Governor Mississippi" },
    { name: "Lynn Fitch", office: "Attorney General of Mississippi", party: "Republican", website: "https://www.ago.state.ms.us", contact: "https://www.ago.state.ms.us/contact/", q: "Lynn Fitch Mississippi Attorney General" },
    { name: "Michael Watson", office: "Secretary of State of Mississippi", party: "Republican", website: "https://www.sos.ms.gov", contact: "https://www.sos.ms.gov/contact", q: "Michael Watson Mississippi Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/ms/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Mississippi State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear MS rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear MS representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert MS representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear MS district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert MS district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Mississippi: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
