...
const STATE = "Idaho";
const STATE_ABBR = "ID";
// Idaho districts are stored zero-padded (ID-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Idaho ${ordinal(num)} Congressional District`,
  ...
  const execDefs = [
    { name: "Brad Little", office: "Governor of Idaho", party: "Republican", website: "https://gov.idaho.gov", contact: "https://gov.idaho.gov/contact/", q: "Brad Little Governor Idaho" },
    { name: "Scott Bedke", office: "Lieutenant Governor of Idaho", party: "Republican", website: "https://ltgov.idaho.gov", contact: "https://ltgov.idaho.gov/contact/", q: "Scott Bedke Lieutenant Governor Idaho" },
    { name: "Raúl Labrador", office: "Attorney General of Idaho", party: "Republican", website: "https://www.ag.idaho.gov", contact: "https://www.ag.idaho.gov/contact-us/", q: "Raúl Labrador Idaho Attorney General" },
    { name: "Phil McGrane", office: "Secretary of State of Idaho", party: "Republican", website: "https://sos.idaho.gov", contact: "https://sos.idaho.gov/contact-us/", q: "Phil McGrane Idaho Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/id/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Idaho State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear ID rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear ID representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert ID representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear ID district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert ID district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Idaho: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
