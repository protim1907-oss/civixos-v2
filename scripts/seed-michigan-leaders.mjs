...
// Michigan districts are stored zero-padded (MI-01 .. MI-13), matching MD/CO/NV/GA.
...
async function main() {
  ...
  const miFederal = legislators.filter((p) => p.terms.at(-1).state === "MI");

  const houseDelegation = miFederal
    ...
    .map((p) => {
      ...
      const site = t.url || "";
      return {
        district_code: `MI-${String(num).padStart(2, "0")}`,
        state: STATE,
        district_number: num,
        name: p.name.official_full,
        title: "U.S. Representative",
        office_label: `Michigan ${ordinal(num)} Congressional District`,
  ...
  const usSenators = miFederal
    .filter((p) => p.terms.at(-1).type === "sen")
    .map((p) => {
      const t = p.terms.at(-1);
      return representative({
        name: p.name.official_full,
        office: "U.S. Senator, Michigan",
        level: "senate",
        website: t.url,
  ...
  const execDefs = [
    { name: "Gretchen Whitmer", office: "Governor of Michigan", party: "Democrat", website: "https://www.michigan.gov/whitmer", contact: "https://www.michigan.gov/whitmer/contact", q: "Gretchen Whitmer Governor Michigan" },
    { name: "Garlin Gilchrist", office: "Lieutenant Governor of Michigan", party: "Democrat", website: "https://www.michigan.gov/ltgov", contact: "https://www.michigan.gov/ltgov/contact", q: "Garlin Gilchrist Michigan Lieutenant Governor" },
    { name: "Dana Nessel", office: "Attorney General of Michigan", party: "Democrat", website: "https://www.michigan.gov/ag", contact: "https://www.michigan.gov/ag/contact", q: "Dana Nessel Michigan Attorney General" },
    { name: "Jocelyn Benson", office: "Secretary of State of Michigan", party: "Democrat", website: "https://www.michigan.gov/sos", contact: "https://www.michigan.gov/sos/contact", q: "Jocelyn Benson Michigan Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/mi/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Michigan State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
        party: partyFull(partyEntry?.name),
        photo: doc.image || "",
        district: `MI-${String(num).padStart(2, "0")}`,
      })
    );
  }
  stateSenators.sort(
  ...
  // ---- Write to Supabase (idempotent: clear MI rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear MI representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert MI representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear MI district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert MI district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Michigan: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
