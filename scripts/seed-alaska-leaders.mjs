...
const STATE = "Alaska";
const STATE_ABBR = "AK";
// Alaska districts are stored zero-padded (AK-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: num === 0 ? `Alaska At-Large Congressional District` : `Alaska ${ordinal(num)} Congressional District`,
  ...
  const execDefs = [
    { name: "Mike Dunleavy", office: "Governor of Alaska", party: "Republican", website: "https://gov.alaska.gov", contact: "https://gov.alaska.gov/contact/", q: "Mike Dunleavy Governor Alaska" },
    { name: "Nancy Dahlstrom", office: "Lieutenant Governor of Alaska", party: "Republican", website: "https://ltgov.alaska.gov", contact: "https://ltgov.alaska.gov/contact/", q: "Nancy Dahlstrom Lieutenant Governor Alaska" },
    { name: "Cori Mills", office: "Attorney General of Alaska", party: "", website: "https://law.alaska.gov", contact: "https://law.alaska.gov/department/contact.html", q: "Cori Mills Alaska Attorney General" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/ak/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: isNumeric ? `Alaska State Senator, District ${num}` : `Alaska State Senator, ${current.district} District`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear AK rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear AK representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert AK representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear AK district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert AK district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Alaska: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
