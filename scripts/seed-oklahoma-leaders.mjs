...
const STATE_ABBR = "OK";
// Oklahoma districts are stored zero-padded (OK-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Oklahoma ${ordinal(num)} Congressional District`,
  ...
  const execDefs = [
    { name: "Kevin Stitt", office: "Governor of Oklahoma", party: "Republican", website: "https://oklahoma.gov/governor.html", contact: "https://oklahoma.gov/governor/contact.html", q: "Kevin Stitt Governor Oklahoma" },
    { name: "Matt Pinnell", office: "Lieutenant Governor of Oklahoma", party: "Republican", website: "https://oklahoma.gov/ltgov.html", contact: "https://oklahoma.gov/ltgov/contact.html", q: "Matt Pinnell Lieutenant Governor Oklahoma" },
    { name: "Gentner Drummond", office: "Attorney General of Oklahoma", party: "Republican", website: "https://www.oag.ok.gov", contact: "https://www.oag.ok.gov/contact", q: "Gentner Drummond Oklahoma Attorney General" },
    { name: "Benjamin Lepak", office: "Secretary of State of Oklahoma", party: "Republican", website: "https://www.sos.ok.gov", contact: "https://www.sos.ok.gov/contact.aspx", q: "Benjamin Lepak Oklahoma Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/ok/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Oklahoma State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear OK rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear OK representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert OK representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear OK district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert OK district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Oklahoma: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
