...
const STATE_ABBR = "SC";
// South Carolina districts are stored zero-padded (SC-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `South Carolina ${ordinal(num)} Congressional District`,
  ...
  // Gov Henry McMaster, Lt. Gov Pamela Evette, AG Alan Wilson (all R).
  const execDefs = [
    { name: "Henry McMaster", office: "Governor of South Carolina", party: "Republican", website: "https://governor.sc.gov", contact: "https://governor.sc.gov/contact-us", q: "Henry McMaster Governor South Carolina" },
    { name: "Pamela Evette", office: "Lieutenant Governor of South Carolina", party: "Republican", website: "https://ltgov.sc.gov", contact: "https://ltgov.sc.gov/contact", q: "Pamela Evette Lieutenant Governor South Carolina" },
    { name: "Alan Wilson", office: "Attorney General of South Carolina", party: "Republican", website: "https://www.scag.gov", contact: "https://www.scag.gov/about-the-office/contact-us/", q: "Alan Wilson South Carolina Attorney General" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/sc/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `South Carolina State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear SC rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear SC representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert SC representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear SC district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert SC district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded South Carolina: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
