...
const STATE_ABBR = "WA";
// Washington districts are stored zero-padded (WA-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Washington ${ordinal(num)} Congressional District`,
  ...
  // Gov Bob Ferguson, Lt. Gov Denny Heck, AG Nick Brown, SoS Steve Hobbs (all D).
  const execDefs = [
    { name: "Bob Ferguson", office: "Governor of Washington", party: "Democrat", website: "https://governor.wa.gov", contact: "https://governor.wa.gov/contact", q: "Bob Ferguson Governor Washington" },
    { name: "Denny Heck", office: "Lieutenant Governor of Washington", party: "Democrat", website: "https://ltgov.wa.gov", contact: "https://ltgov.wa.gov/contact", q: "Denny Heck Lieutenant Governor Washington" },
    { name: "Nick Brown", office: "Attorney General of Washington", party: "Democrat", website: "https://www.atg.wa.gov", contact: "https://www.atg.wa.gov/contact-us", q: "Nick Brown Washington Attorney General" },
    { name: "Steve Hobbs", office: "Secretary of State of Washington", party: "Democrat", website: "https://www.sos.wa.gov", contact: "https://www.sos.wa.gov/about-office/contact-us", q: "Steve Hobbs Washington Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/wa/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Washington State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear WA rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear WA representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert WA representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear WA district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert WA district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Washington: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
