...
const STATE = "Oregon";
const STATE_ABBR = "OR";
// Oregon districts are stored zero-padded (OR-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Oregon ${ordinal(num)} Congressional District`,
  ...
  // Statewide executives (Wikipedia headshots). Verified current 2026-08-05. Oregon has NO Lieutenant Governor (the SoS is the #2 successor).
  const execDefs = [
    { name: "Tina Kotek", office: "Governor of Oregon", party: "Democrat", website: "https://www.oregon.gov/gov", contact: "https://www.oregon.gov/gov/pages/share-your-opinion.aspx", q: "Tina Kotek Governor Oregon" },
    { name: "Tobias Read", office: "Secretary of State of Oregon", party: "Democrat", website: "https://sos.oregon.gov", contact: "https://sos.oregon.gov/Pages/contactus.aspx", q: "Tobias Read Oregon Secretary of State" },
    { name: "Dan Rayfield", office: "Attorney General of Oregon", party: "Democrat", website: "https://www.doj.state.or.us", contact: "https://www.doj.state.or.us/about-us/contact-us/", q: "Dan Rayfield Oregon Attorney General" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/or/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Oregon State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear OR rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear OR representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert OR representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear OR district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert OR district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Oregon: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
