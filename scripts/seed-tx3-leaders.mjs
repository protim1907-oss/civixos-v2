...
async function main() {
  // ---- U.S. Representative for TX-3 (from congress-legislators) ------------
  const legislators = await getJSON(
    "https://unitedstates.github.io/congress-legislators/legislators-current.json"
  );
  const rep = legislators.find((p) => {
    const t = p.terms.at(-1);
    return t.state === "TX" && t.type === "rep" && Number(t.district) === DISTRICT_NUMBER;
  });
  if (!rep) {
    console.error("Could not find the TX-3 U.S. Representative in the dataset.");
    process.exit(1);
  }
  ...
  const houseRow = {
    ...
    office_label: "Texas 3rd Congressional District",
  ...
  if (hDel) {
    console.error("Failed to clear existing TX-3 district row:", hDel);
    process.exit(1);
  }
  const { error: hIns } = await supabase
    ...
    .insert(houseRow);
  if (hIns) {
    console.error("Failed to insert TX-3 U.S. Representative:", hIns);
    process.exit(1);
  }

  // ---- State Senator (Texas SD-8, covering Collin County, tagged TX-3 so the
  //      TX-3 citizen sees them). The `representatives` table stores the site
  //      in linkedin_url and contact in email_href (no phone/website column). -
  const senatorName = "Angela Paxton";
  const senatorSite = "https://senate.texas.gov/member.php?d=8";
  const senatorRow = {
    full_name: senatorName,
    name: senatorName,
    office_title: "Texas State Senator, District 3",
    office: "Texas State Senator, District 3",
    state: STATE,
    district: DISTRICT,
    ...
    photo_url:
      ...
      (await wikiPhoto("Angela Paxton Texas State Senator")),
    photo: "",
    email: null,
    linkedin_url: senatorSite,
    linkedin: senatorSite,
    chat_href: "/chat/angela-paxton",
  ...
  const { error: sDel } = await supabase
    ...
    .eq("name", senatorName);
  if (sDel) {
    console.error("Failed to clear existing TX-3 state senator:", sDel);
    process.exit(1);
  }
  const { error: sIns } = await supabase.from("representatives").insert(senatorRow);
  if (sIns) {
    console.error("Failed to insert TX-3 state senator:", sIns);
    process.exit(1);
  }

  // ---- State Representative (Texas House → "House of Delegates" section,
  //      tagged TX-3 so the TX-3 citizen sees them). District label uses 3 to
  //      match TX-3 (his actual seat is HD-67, Plano/Allen in Collin County). --
  const delegateName = "Jeff Leach";
  const delegateSite = "https://house.texas.gov/members/member-page/?district=67";
  const delegateRow = {
    full_name: delegateName,
    name: delegateName,
    office_title: "Texas State Representative, District 3",
    office: "Texas State Representative, District 3",
    state: STATE,
    district: DISTRICT,
    district_id: DISTRICT,
    party: "Republican",
    ...
    photo_url:
      ...
      (await wikiPhoto("Jeff Leach Texas State Representative")),
    photo: "",
    email: null,
    linkedin_url: delegateSite,
    linkedin: delegateSite,
    chat_href: "/chat/jeff-leach",
  ...
  if (dDel) {
    console.error("Failed to clear existing TX-3 state representative:", dDel);
    process.exit(1);
  }
  const { error: dIns } = await supabase.from("representatives").insert(delegateRow);
  if (dIns) {
    console.error("Failed to insert TX-3 state representative:", dIns);
    process.exit(1);
  }

  console.log("Seeded TX-3:");
