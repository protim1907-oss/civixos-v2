...
const STATE = "Arizona";
const STATE_ABBR = "AZ";
// Arizona districts are stored zero-padded (AZ-01 .. AZ-09).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Arizona ${ordinal(num)} Congressional District`,
  ...
  // Arizona has no Lieutenant Governor office yet (created for the 2026 cycle,
  // first LG takes office 2027). It elects Governor, Secretary of State, and
  // Attorney General. Verified current 2026-08-05: Hobbs, Fontes, Mayes (all D,
  // took office 2023-01-02).
  const execDefs = [
    { name: "Katie Hobbs", office: "Governor of Arizona", party: "Democrat", website: "https://azgovernor.gov", contact: "https://azgovernor.gov/engage/form/contact-governor-hobbs", q: "Katie Hobbs Governor Arizona" },
    { name: "Adrian Fontes", office: "Secretary of State of Arizona", party: "Democrat", website: "https://azsos.gov", contact: "https://azsos.gov/about-office/contact-us", q: "Adrian Fontes Arizona Secretary of State" },
    { name: "Kris Mayes", office: "Attorney General of Arizona", party: "Democrat", website: "https://www.azag.gov", contact: "https://www.azag.gov/contact-us", q: "Kris Mayes Arizona Attorney General" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/az/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Arizona State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear AZ rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear AZ representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert AZ representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear AZ district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert AZ district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Arizona: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
