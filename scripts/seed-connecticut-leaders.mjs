...
const STATE_ABBR = "CT";
// Connecticut districts are stored zero-padded (CT-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Connecticut ${ordinal(num)} Congressional District`,
  ...
  const usSenators = stFederal
    ...
    .map((p) => {
      ...
      return representative({
        name: p.name.official_full,
        office: "U.S. Senator, Connecticut",
        level: "senate",
        website: t.url,
  ...
  // Statewide executives (Wikipedia headshots). Verified current 2026-08-05. Lamont/Bysiewicz/Tong/Thomas (all D).
  const execDefs = [
    { name: "Ned Lamont", office: "Governor of Connecticut", party: "Democrat", website: "https://portal.ct.gov/governor", contact: "https://portal.ct.gov/governor/contact/email-the-governor", q: "Ned Lamont Governor Connecticut" },
    { name: "Susan Bysiewicz", office: "Lieutenant Governor of Connecticut", party: "Democrat", website: "https://portal.ct.gov/ltgovernor", contact: "https://portal.ct.gov/ltgovernor/contact-us", q: "Susan Bysiewicz Lieutenant Governor Connecticut" },
    { name: "William Tong", office: "Attorney General of Connecticut", party: "Democrat", website: "https://portal.ct.gov/ag", contact: "https://portal.ct.gov/ag/contact-the-attorney-generals-office", q: "William Tong Connecticut Attorney General" },
    { name: "Stephanie Thomas", office: "Secretary of the State of Connecticut", party: "Democrat", website: "https://portal.ct.gov/sots", contact: "https://portal.ct.gov/sots/contact-us", q: "Stephanie Thomas Connecticut Secretary of the State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/ct/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Connecticut State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear CT rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear CT representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert CT representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear CT district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert CT district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Connecticut: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
