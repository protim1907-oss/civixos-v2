...
const STATE = "Utah";
const STATE_ABBR = "UT";
// Utah districts are stored zero-padded (UT-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Utah ${ordinal(num)} Congressional District`,
  ...
  const execDefs = [
    { name: "Spencer Cox", office: "Governor of Utah", party: "Republican", website: "https://governor.utah.gov", contact: "https://governor.utah.gov/contact/", q: "Spencer Cox Governor Utah" },
    { name: "Deidre Henderson", office: "Lieutenant Governor of Utah", party: "Republican", website: "https://ltgovernor.utah.gov", contact: "https://ltgovernor.utah.gov/contact/", q: "Deidre Henderson Lieutenant Governor Utah" },
    { name: "Derek Brown", office: "Attorney General of Utah", party: "Republican", website: "https://attorneygeneral.utah.gov", contact: "https://attorneygeneral.utah.gov/contact/", q: "Derek Brown Utah Attorney General" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/ut/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Utah State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear UT rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear UT representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert UT representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear UT district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert UT district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Utah: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
