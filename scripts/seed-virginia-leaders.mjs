...
// Virginia districts are stored zero-padded (VA-01 .. VA-11), matching MD/CO/NV/GA/MI/NY.
...
async function main() {
  ...
  const vaFederal = legislators.filter((p) => p.terms.at(-1).state === "VA");

  const houseDelegation = vaFederal
    ...
    .map((p) => {
      ...
      const site = t.url || "";
      return {
        district_code: `VA-${String(num).padStart(2, "0")}`,
        state: STATE,
        district_number: num,
        name: p.name.official_full,
        title: "U.S. Representative",
        office_label: `Virginia ${ordinal(num)} Congressional District`,
  ...
  // Virginia's administration changed on 2026-01-17 (verified 2026-07-31).
  // Virginia elects only Governor, Lt. Governor, and Attorney General statewide.
  const execDefs = [
    { name: "Abigail Spanberger", office: "Governor of Virginia", party: "Democrat", website: "https://www.governor.virginia.gov", contact: "https://www.governor.virginia.gov/constituent-services/communicating-with-the-governors-office/", q: "Abigail Spanberger Governor Virginia" },
    { name: "Ghazala Hashmi", office: "Lieutenant Governor of Virginia", party: "Democrat", website: "https://www.ltgov.virginia.gov", contact: "https://www.ltgov.virginia.gov/contact/", q: "Ghazala Hashmi Virginia Lieutenant Governor" },
    { name: "Jay Jones", office: "Attorney General of Virginia", party: "Democrat", website: "https://www.oag.state.va.us", contact: "https://www.oag.state.va.us/citizen-resources/contact-us", q: "Jay Jones Virginia Attorney General" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/va/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Virginia State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
        party: partyFull(partyEntry?.name),
        photo: doc.image || "",
        district: `VA-${String(num).padStart(2, "0")}`,
      })
    );
  }
  stateSenators.sort(
  ...
  // ---- Write to Supabase (idempotent: clear VA rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear VA representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert VA representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear VA district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert VA district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Virginia: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
