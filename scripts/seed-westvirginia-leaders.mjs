...
const STATE = "West Virginia";
const STATE_ABBR = "WV";
// West Virginia districts are stored zero-padded (WV-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `West Virginia ${ordinal(num)} Congressional District`,
  ...
  const usSenators = stFederal
    ...
    .map((p) => {
      ...
      return representative({
        name: p.name.official_full,
        office: "U.S. Senator, West Virginia",
        level: "senate",
        website: t.url,
  ...
  // Statewide executives (Wikipedia headshots). Verified current 2026-09-01.
  const execDefs = [
    { name: "Patrick Morrisey", office: "Governor of West Virginia", party: "Republican", website: "https://governor.wv.gov", contact: "https://governor.wv.gov/Pages/Contact-the-Governor.aspx", q: "Patrick Morrisey Governor West Virginia" },
    { name: "Randy Smith", office: "Lieutenant Governor of West Virginia", party: "Republican", website: "https://www.wvlegislature.gov/Senate1/lawmaker.cfm", contact: "https://www.wvlegislature.gov", q: "Randy Smith West Virginia Senate President" },
    { name: "JB McCuskey", office: "Attorney General of West Virginia", party: "Republican", website: "https://ago.wv.gov", contact: "https://ago.wv.gov/about/Pages/Contact-Us.aspx", q: "JB McCuskey West Virginia Attorney General" },
    { name: "Kris Warner", office: "Secretary of State of West Virginia", party: "Republican", website: "https://sos.wv.gov", contact: "https://sos.wv.gov/about/Pages/ContactUs.aspx", q: "Kris Warner West Virginia Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/wv/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `West Virginia State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear WV rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear WV representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert WV representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear WV district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert WV district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded West Virginia: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
