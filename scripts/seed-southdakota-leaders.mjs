...
const STATE_ABBR = "SD";
// South Dakota districts are stored zero-padded (SD-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: num === 0 ? `South Dakota At-Large Congressional District` : `South Dakota ${ordinal(num)} Congressional District`,
  ...
  const usSenators = stFederal
    ...
    .map((p) => {
      ...
      return representative({
        name: p.name.official_full,
        office: "U.S. Senator, South Dakota",
        level: "senate",
        website: t.url,
  ...
  const execDefs = [
    { name: "Larry Rhoden", office: "Governor of South Dakota", party: "Republican", website: "https://governor.sd.gov", contact: "https://governor.sd.gov/contact/", q: "Larry Rhoden Governor South Dakota" },
    { name: "Tony Venhuizen", office: "Lieutenant Governor of South Dakota", party: "Republican", website: "https://ltgov.sd.gov", contact: "https://ltgov.sd.gov/", q: "Tony Venhuizen Lieutenant Governor South Dakota" },
    { name: "Marty Jackley", office: "Attorney General of South Dakota", party: "Republican", website: "https://atg.sd.gov", contact: "https://atg.sd.gov/contact.aspx", q: "Marty Jackley South Dakota Attorney General" },
    { name: "Monae Johnson", office: "Secretary of State of South Dakota", party: "Republican", website: "https://sdsos.gov", contact: "https://sdsos.gov/general-information/contact-us.aspx", q: "Monae Johnson South Dakota Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/sd/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `South Dakota State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear SD rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear SD representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert SD representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear SD district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert SD district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded South Dakota: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
