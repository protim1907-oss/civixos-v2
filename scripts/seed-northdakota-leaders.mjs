...
const STATE_ABBR = "ND";
// North Dakota districts are stored zero-padded (ND-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: num === 0 ? `North Dakota At-Large Congressional District` : `North Dakota ${ordinal(num)} Congressional District`,
  ...
  const execDefs = [
    { name: "Kelly Armstrong", office: "Governor of North Dakota", party: "Republican", website: "https://www.governor.nd.gov", contact: "https://www.governor.nd.gov/contact", q: "Kelly Armstrong Governor North Dakota" },
    { name: "Michelle Strinden", office: "Lieutenant Governor of North Dakota", party: "Republican", website: "https://www.governor.nd.gov/lieutenant-governor", contact: "https://www.governor.nd.gov/contact", q: "Michelle Strinden Lieutenant Governor North Dakota" },
    { name: "Drew Wrigley", office: "Attorney General of North Dakota", party: "Republican", website: "https://attorneygeneral.nd.gov", contact: "https://attorneygeneral.nd.gov/contact", q: "Drew Wrigley North Dakota Attorney General" },
    { name: "Michael Howe", office: "Secretary of State of North Dakota", party: "Republican", website: "https://sos.nd.gov", contact: "https://sos.nd.gov/contact-us", q: "Michael Howe North Dakota Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/nd/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `North Dakota State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear ND rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear ND representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert ND representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear ND district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert ND district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded North Dakota: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
