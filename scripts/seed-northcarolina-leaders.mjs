...
const STATE_ABBR = "NC";
// North Carolina districts are stored zero-padded (NC-01 .. NC-14), matching MD/CO/NV/GA/MI/NY/VA.
...
async function main() {
  ...
  const ncFederal = legislators.filter((p) => p.terms.at(-1).state === STATE_ABBR);

  const houseDelegation = ncFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `North Carolina ${ordinal(num)} Congressional District`,
  ...
  // North Carolina's Council of State turned over on 2025-01-01. Governor,
  // Lt. Governor, Attorney General, and Secretary of State are elected statewide.
  const execDefs = [
    { name: "Josh Stein", office: "Governor of North Carolina", party: "Democrat", website: "https://governor.nc.gov", contact: "https://governor.nc.gov/contact", q: "Josh Stein Governor North Carolina" },
    { name: "Rachel Hunt", office: "Lieutenant Governor of North Carolina", party: "Democrat", website: "https://ltgov.nc.gov", contact: "https://ltgov.nc.gov/contact", q: "Rachel Hunt Lieutenant Governor North Carolina" },
    { name: "Jeff Jackson", office: "Attorney General of North Carolina", party: "Democrat", website: "https://ncdoj.gov", contact: "https://ncdoj.gov/contact-us/", q: "Jeff Jackson North Carolina Attorney General" },
    { name: "Elaine Marshall", office: "Secretary of State of North Carolina", party: "Democrat", website: "https://www.sosnc.gov", contact: "https://www.sosnc.gov/contact", q: "Elaine Marshall North Carolina Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/nc/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `North Carolina State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear NC rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear NC representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert NC representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear NC district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert NC district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded North Carolina: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
