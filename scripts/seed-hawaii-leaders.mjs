...
const STATE = "Hawaii";
const STATE_ABBR = "HI";
// Hawaii districts are stored zero-padded (HI-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Hawaii ${ordinal(num)} Congressional District`,
  ...
  const execDefs = [
    { name: "Josh Green", office: "Governor of Hawaii", party: "Democrat", website: "https://governor.hawaii.gov", contact: "https://governor.hawaii.gov/contact-us/", q: "Josh Green Governor Hawaii" },
    { name: "Sylvia Luke", office: "Lieutenant Governor of Hawaii", party: "Democrat", website: "https://ltgov.hawaii.gov", contact: "https://ltgov.hawaii.gov/contact/", q: "Sylvia Luke Lieutenant Governor Hawaii" },
    { name: "Anne Lopez", office: "Attorney General of Hawaii", party: "Democrat", website: "https://ag.hawaii.gov", contact: "https://ag.hawaii.gov/about-us/contact-us/", q: "Anne Lopez Hawaii Attorney General" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/hi/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Hawaii State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear HI rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear HI representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert HI representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear HI district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert HI district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Hawaii: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
