...
const STATE_ABBR = "LA";
// Louisiana districts are stored zero-padded (LA-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Louisiana ${ordinal(num)} Congressional District`,
  ...
  const execDefs = [
    { name: "Jeff Landry", office: "Governor of Louisiana", party: "Republican", website: "https://gov.louisiana.gov", contact: "https://gov.louisiana.gov/contact", q: "Jeff Landry Governor Louisiana" },
    { name: "Billy Nungesser", office: "Lieutenant Governor of Louisiana", party: "Republican", website: "https://www.crt.state.la.us/ltgovernor/", contact: "https://www.crt.state.la.us/ltgovernor/contact/", q: "Billy Nungesser Lieutenant Governor Louisiana" },
    { name: "Liz Murrill", office: "Attorney General of Louisiana", party: "Republican", website: "https://www.ag.state.la.us", contact: "https://www.ag.state.la.us/contact", q: "Liz Murrill Louisiana Attorney General" },
    { name: "Nancy Landry", office: "Secretary of State of Louisiana", party: "Republican", website: "https://www.sos.la.gov", contact: "https://www.sos.la.gov/Pages/ContactUs.aspx", q: "Nancy Landry Louisiana Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/la/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Louisiana State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear LA rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear LA representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert LA representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear LA district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert LA district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Louisiana: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
