...
const STATE = "Maine";
const STATE_ABBR = "ME";
// Maine districts are stored zero-padded (ME-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Maine ${ordinal(num)} Congressional District`,
  ...
  const execDefs = [
    { name: "Janet Mills", office: "Governor of Maine", party: "Democrat", website: "https://www.maine.gov/governor/mills/", contact: "https://www.maine.gov/governor/mills/contact", q: "Janet Mills Governor Maine" },
    { name: "Aaron Frey", office: "Attorney General of Maine", party: "Democrat", website: "https://www.maine.gov/ag/", contact: "https://www.maine.gov/ag/contact.shtml", q: "Aaron Frey Maine Attorney General" },
    { name: "Shenna Bellows", office: "Secretary of State of Maine", party: "Democrat", website: "https://www.maine.gov/sos/", contact: "https://www.maine.gov/sos/contact.html", q: "Shenna Bellows Maine Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/me/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Maine State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear ME rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear ME representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert ME representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear ME district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert ME district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Maine: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
