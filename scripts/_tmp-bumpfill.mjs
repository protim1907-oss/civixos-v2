...
const { data: msgd } = await s.from("outreach_messages").select("lead_id,status").eq("campaign_id",c.id);
const done = new Set(msgd.map(m=>m.lead_id));
const approvedNow = msgd.filter(m=>m.status==="approved").length;
const need = Math.max(0, 50 - approvedNow);
...
const next = bump.slice(0,need);
console.log(`approved now=${approvedNow}, need ${need} more, generating for ${next.length}`);
...
console.log(`Added ${ok} (fail ${fail}, sign-off issues ${bad}). Bump totals: ${JSON.stringify(by)}`);
