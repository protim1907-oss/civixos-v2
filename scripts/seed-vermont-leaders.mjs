...
const STATE = "Vermont";
const STATE_ABBR = "VT";
// Vermont districts are stored zero-padded (VT-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: num === 0 ? `Vermont At-Large Congressional District` : `Vermont ${ordinal(num)} Congressional District`,
  ...
  // Statewide executives (Wikipedia headshots). Verified current 2026-09-05.
  const execDefs = [
    { name: "Phil Scott", office: "Governor of Vermont", party: "Republican", website: "https://governor.vermont.gov", contact: "https://governor.vermont.gov/contact", q: "Phil Scott Governor Vermont" },
    { name: "John Rodgers", office: "Lieutenant Governor of Vermont", party: "Republican", website: "https://ltgov.vermont.gov", contact: "https://ltgov.vermont.gov/contact", q: "John Rodgers Vermont Lieutenant Governor" },
    { name: "Charity Clark", office: "Attorney General of Vermont", party: "Democrat", website: "https://ago.vermont.gov", contact: "https://ago.vermont.gov/about-us/contact-us", q: "Charity Clark Vermont Attorney General" },
    { name: "Sarah Copeland Hanzas", office: "Secretary of State of Vermont", party: "Democrat", website: "https://sos.vermont.gov", contact: "https://sos.vermont.gov/contact-us/", q: "Sarah Copeland Hanzas Vermont Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/vt/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: isNumeric ? `Vermont State Senator, District ${num}` : `Vermont State Senator, ${current.district} District`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear VT rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear VT representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert VT representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear VT district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert VT district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Vermont: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
