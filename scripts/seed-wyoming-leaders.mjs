...
const STATE = "Wyoming";
const STATE_ABBR = "WY";
// Wyoming districts are stored zero-padded (WY-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: num === 0 ? `Wyoming At-Large Congressional District` : `Wyoming ${ordinal(num)} Congressional District`,
  ...
  const execDefs = [
    { name: "Mark Gordon", office: "Governor of Wyoming", party: "Republican", website: "https://governor.wyo.gov", contact: "https://governor.wyo.gov/contact-us", q: "Mark Gordon Governor Wyoming" },
    { name: "Chuck Gray", office: "Secretary of State of Wyoming", party: "Republican", website: "https://sos.wyo.gov", contact: "https://sos.wyo.gov/AboutUs/ContactUs.aspx", q: "Chuck Gray Wyoming Secretary of State" },
    { name: "Keith Kautz", office: "Attorney General of Wyoming", party: "", website: "https://ag.wyo.gov", contact: "https://ag.wyo.gov/contact", q: "Keith Kautz Wyoming Attorney General" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/wy/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Wyoming State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear WY rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear WY representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert WY representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear WY district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert WY district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Wyoming: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
