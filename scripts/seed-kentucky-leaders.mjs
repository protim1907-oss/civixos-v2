...
const STATE_ABBR = "KY";
// Kentucky districts are stored zero-padded (KY-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Kentucky ${ordinal(num)} Congressional District`,
  ...
  // Statewide executives (Wikipedia headshots). Verified current 2026-08-05. Beshear + J. Coleman (D); AG R. Coleman + SoS Adams (R).
  const execDefs = [
    { name: "Andy Beshear", office: "Governor of Kentucky", party: "Democrat", website: "https://governor.ky.gov", contact: "https://governor.ky.gov/contact-us", q: "Andy Beshear Governor Kentucky" },
    { name: "Jacqueline Coleman", office: "Lieutenant Governor of Kentucky", party: "Democrat", website: "https://ltgovernor.ky.gov", contact: "https://ltgovernor.ky.gov/Pages/contact.aspx", q: "Jacqueline Coleman Lieutenant Governor Kentucky" },
    { name: "Russell Coleman", office: "Attorney General of Kentucky", party: "Republican", website: "https://www.ag.ky.gov", contact: "https://www.ag.ky.gov/Contact/Pages/default.aspx", q: "Russell Coleman Kentucky Attorney General" },
    { name: "Michael Adams", office: "Secretary of State of Kentucky", party: "Republican", website: "https://www.sos.ky.gov", contact: "https://www.sos.ky.gov/Pages/contactus.aspx", q: "Michael Adams Kentucky Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/ky/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Kentucky State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear KY rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear KY representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert KY representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear KY district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert KY district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Kentucky: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
