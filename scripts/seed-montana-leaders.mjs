...
const STATE = "Montana";
const STATE_ABBR = "MT";
// Montana districts are stored zero-padded (MT-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Montana ${ordinal(num)} Congressional District`,
  ...
  const execDefs = [
    { name: "Greg Gianforte", office: "Governor of Montana", party: "Republican", website: "https://governor.mt.gov", contact: "https://governor.mt.gov/contact/", q: "Greg Gianforte Governor Montana" },
    { name: "Kristen Juras", office: "Lieutenant Governor of Montana", party: "Republican", website: "https://ltgov.mt.gov", contact: "https://ltgov.mt.gov/contact/", q: "Kristen Juras Lieutenant Governor Montana" },
    { name: "Austin Knudsen", office: "Attorney General of Montana", party: "Republican", website: "https://dojmt.gov", contact: "https://dojmt.gov/contact/", q: "Austin Knudsen Montana Attorney General" },
    { name: "Christi Jacobsen", office: "Secretary of State of Montana", party: "Republican", website: "https://sosmt.gov", contact: "https://sosmt.gov/contact/", q: "Christi Jacobsen Montana Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/mt/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Montana State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear MT rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear MT representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert MT representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear MT district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert MT district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Montana: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
