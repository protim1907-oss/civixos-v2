...
const STATE = "Indiana";
const STATE_ABBR = "IN";
// Indiana districts are stored zero-padded (IN-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Indiana ${ordinal(num)} Congressional District`,
  ...
  // Gov Mike Braun, Lt. Gov Micah Beckwith, AG Todd Rokita (all R, since Jan 2025).
  const execDefs = [
    { name: "Mike Braun", office: "Governor of Indiana", party: "Republican", website: "https://www.in.gov/gov", contact: "https://www.in.gov/gov/contact-us/", q: "Mike Braun Governor Indiana" },
    { name: "Micah Beckwith", office: "Lieutenant Governor of Indiana", party: "Republican", website: "https://www.in.gov/lg", contact: "https://www.in.gov/lg/contact-us/", q: "Micah Beckwith Lieutenant Governor Indiana" },
    { name: "Todd Rokita", office: "Attorney General of Indiana", party: "Republican", website: "https://www.in.gov/attorneygeneral", contact: "https://www.in.gov/attorneygeneral/contact-us/", q: "Todd Rokita Indiana Attorney General" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/in/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Indiana State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear IN rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear IN representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert IN representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear IN district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert IN district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Indiana: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
