...
const STATE_ABBR = "TN";
// Tennessee districts are stored zero-padded (TN-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Tennessee ${ordinal(num)} Congressional District`,
  ...
  // Statewide officials (Wikipedia headshots). Verified current 2026-08-05.
  // Note: TN is one of two states where only the Governor is popularly elected;
  // the AG is appointed by the state Supreme Court and the SoS by the legislature.
  const execDefs = [
    { name: "Bill Lee", office: "Governor of Tennessee", party: "Republican", website: "https://www.tn.gov/governor", contact: "https://www.tn.gov/governor/contact-us.html", q: "Bill Lee Governor Tennessee" },
    { name: "Jonathan Skrmetti", office: "Attorney General of Tennessee", party: "Republican", website: "https://www.tn.gov/attorneygeneral", contact: "https://www.tn.gov/attorneygeneral/about-the-office/contact.html", q: "Jonathan Skrmetti Tennessee Attorney General" },
    { name: "Tre Hargett", office: "Secretary of State of Tennessee", party: "Republican", website: "https://sos.tn.gov", contact: "https://sos.tn.gov/contact", q: "Tre Hargett Tennessee Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/tn/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Tennessee State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear TN rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear TN representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert TN representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear TN district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert TN district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Tennessee: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
