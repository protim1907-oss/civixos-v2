...
const STATE_ABBR = "NE";
// Nebraska districts are stored zero-padded (NE-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Nebraska ${ordinal(num)} Congressional District`,
  ...
  const execDefs = [
    { name: "Jim Pillen", office: "Governor of Nebraska", party: "Republican", website: "https://governor.nebraska.gov", contact: "https://governor.nebraska.gov/contact-governor-pillen", q: "Jim Pillen Governor Nebraska" },
    { name: "Joe Kelly", office: "Lieutenant Governor of Nebraska", party: "Republican", website: "https://ltgov.nebraska.gov", contact: "https://ltgov.nebraska.gov", q: "Joe Kelly Nebraska Lieutenant Governor" },
    { name: "Mike Hilgers", office: "Attorney General of Nebraska", party: "Republican", website: "https://ago.nebraska.gov", contact: "https://ago.nebraska.gov/contact", q: "Mike Hilgers Nebraska Attorney General" },
    { name: "Bob Evnen", office: "Secretary of State of Nebraska", party: "Republican", website: "https://sos.nebraska.gov", contact: "https://sos.nebraska.gov/contact", q: "Bob Evnen Nebraska Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/ne/legislature"
    )
  ...
  for (const doc of docs) {
    if (!doc) continue;
    const upperRoles = (doc.roles || []).filter((r) => r.type === "upper" || r.type === "legislature");
    const asDateStr = (v) =>
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Nebraska State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear NE rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear NE representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert NE representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear NE district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert NE district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Nebraska: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
