...
const STATE_ABBR = "AR";
// Arkansas districts are stored zero-padded (AR-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Arkansas ${ordinal(num)} Congressional District`,
  ...
  // Statewide executives (Wikipedia headshots). Verified current 2026-08-27.
  const execDefs = [
    { name: "Sarah Huckabee Sanders", office: "Governor of Arkansas", party: "Republican", website: "https://governor.arkansas.gov", contact: "https://governor.arkansas.gov/contact-us/", q: "Sarah Huckabee Sanders Governor Arkansas" },
    { name: "Leslie Rutledge", office: "Lieutenant Governor of Arkansas", party: "Republican", website: "https://ltgovernor.arkansas.gov", contact: "https://ltgovernor.arkansas.gov/contact/", q: "Leslie Rutledge Lieutenant Governor Arkansas" },
    { name: "Tim Griffin", office: "Attorney General of Arkansas", party: "Republican", website: "https://arkansasag.gov", contact: "https://arkansasag.gov/contact-us/", q: "Tim Griffin Arkansas Attorney General" },
    { name: "Cole Jester", office: "Secretary of State of Arkansas", party: "Republican", website: "https://www.sos.arkansas.gov", contact: "https://www.sos.arkansas.gov/contact/", q: "Cole Jester Arkansas Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/ar/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Arkansas State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // OpenStates carries a stale end_date (2023-01-08) on Bart Hester's Senate
  // District 1 role, so the current-window filter drops him even though he is the
  // sitting senator (Senate President Pro Tempore). Add him back if missing.
  if (!stateSenators.some((s) => s.district === `${STATE_ABBR}-01`)) {
    stateSenators.push(
      representative({
        name: "Bart Hester",
        office: "Arkansas State Senator, District 1",
        level: "State Senate",
        website: "https://senate.arkansas.gov/senators/s01/",
        contact: "https://senate.arkansas.gov/senators/s01/",
        party: "Republican",
        photo: "",
        district: `${STATE_ABBR}-01`,
      })
    );
  }
  stateSenators.sort(
  ...
  // ---- Write to Supabase (idempotent: clear AR rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear AR representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert AR representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear AR district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert AR district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Arkansas: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
