...
// NOTE: Run sql/allow-ohio-district.sql in the Supabase SQL Editor FIRST.
// The profiles table has a trigger (enforce_tx_ca_district) that rejects any
// district outside the allowed states, so this profile upsert fails until OH
// is added to that trigger.
const DEMO = {
  email: "oh11.demo@civix250.com",
  password: "OH11-Demo-2026!",
  full_name: "OH Demo Citizen",
  role: "citizen",
  district: "OH-11",
  state: "Ohio",
  street_address: "16700 Chagrin Blvd",
  city: "Shaker Heights",
  zip_code: "44120",
};
  ...
  if (profileError) {
    console.error(
      ...
      "\n\nIf this is a check_violation on the district, run " +
        "sql/allow-ohio-district.sql in the Supabase SQL Editor first."
    );
    process.exit(1);
  }

  console.log("\nOH-11 demo account ready:");
