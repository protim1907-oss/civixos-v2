...
const STATE_ABBR = "MO";
// Missouri districts are stored zero-padded (MO-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Missouri ${ordinal(num)} Congressional District`,
  ...
  // Gov Mike Kehoe, Lt. Gov David Wasinger, AG Catherine Hanaway (all R; Hanaway
  // sworn in 2025-09-08 after Andrew Bailey left for the FBI).
  const execDefs = [
    { name: "Mike Kehoe", office: "Governor of Missouri", party: "Republican", website: "https://governor.mo.gov", contact: "https://governor.mo.gov/contact-governor-kehoe", q: "Mike Kehoe Governor Missouri" },
    { name: "David Wasinger", office: "Lieutenant Governor of Missouri", party: "Republican", website: "https://ltgov.mo.gov", contact: "https://ltgov.mo.gov/contact", q: "David Wasinger Lieutenant Governor Missouri" },
    { name: "Catherine Hanaway", office: "Attorney General of Missouri", party: "Republican", website: "https://ago.mo.gov", contact: "https://ago.mo.gov/contact-us", q: "Catherine Hanaway Missouri Attorney General" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/mo/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Missouri State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear MO rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear MO representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert MO representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear MO district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert MO district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Missouri: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
