...
const STATE = "Alabama";
const STATE_ABBR = "AL";
// Alabama districts are stored zero-padded (AL-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Alabama ${ordinal(num)} Congressional District`,
  ...
  const execDefs = [
    { name: "Kay Ivey", office: "Governor of Alabama", party: "Republican", website: "https://governor.alabama.gov", contact: "https://governor.alabama.gov/contact/", q: "Kay Ivey Governor Alabama" },
    { name: "Will Ainsworth", office: "Lieutenant Governor of Alabama", party: "Republican", website: "https://ltgov.alabama.gov", contact: "https://ltgov.alabama.gov/contact/", q: "Will Ainsworth Lieutenant Governor Alabama" },
    { name: "Steve Marshall", office: "Attorney General of Alabama", party: "Republican", website: "https://www.alabamaag.gov", contact: "https://www.alabamaag.gov/contact/", q: "Steve Marshall Alabama Attorney General" },
    { name: "Wes Allen", office: "Secretary of State of Alabama", party: "Republican", website: "https://www.sos.alabama.gov", contact: "https://www.sos.alabama.gov/contact-us", q: "Wes Allen Alabama Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/al/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Alabama State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear AL rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear AL representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert AL representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear AL district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert AL district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Alabama: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
