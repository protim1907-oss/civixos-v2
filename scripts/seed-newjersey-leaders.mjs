...
const STATE_ABBR = "NJ";
// New Jersey districts are stored zero-padded (NJ-01 .. NJ-12).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `New Jersey ${ordinal(num)} Congressional District`,
  ...
  // New Jersey elects Governor + Lt. Governor on one ticket; the Attorney
  // General is APPOINTED (not elected), so only the two elected execs are
  // seeded. Verified current 2026-08-05: Gov Mikie Sherrill (D) and Lt. Gov
  // Dale Caldwell (D), sworn in 2026-01-20.
  const execDefs = [
    { name: "Mikie Sherrill", office: "Governor of New Jersey", party: "Democrat", website: "https://www.nj.gov/governor", contact: "https://www.nj.gov/governor/contact/", q: "Mikie Sherrill Governor New Jersey" },
    { name: "Dale Caldwell", office: "Lieutenant Governor of New Jersey", party: "Democrat", website: "https://www.nj.gov/governor/admin/lt/", contact: "https://www.nj.gov/governor/contact/", q: "Dale Caldwell Lieutenant Governor New Jersey" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/nj/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `New Jersey State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear NJ rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear NJ representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert NJ representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear NJ district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert NJ district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded New Jersey: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
