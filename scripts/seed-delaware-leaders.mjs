...
const STATE_ABBR = "DE";
// Delaware districts are stored zero-padded (DE-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: num === 0 ? `Delaware At-Large Congressional District` : `Delaware ${ordinal(num)} Congressional District`,
  ...
  const execDefs = [
    { name: "Matt Meyer", office: "Governor of Delaware", party: "Democrat", website: "https://governor.delaware.gov", contact: "https://governor.delaware.gov/contact-the-governor/", q: "Matt Meyer Governor Delaware" },
    { name: "Kyle Evans Gay", office: "Lieutenant Governor of Delaware", party: "Democrat", website: "https://ltgov.delaware.gov", contact: "https://ltgov.delaware.gov/contact/", q: "Kyle Evans Gay Lieutenant Governor Delaware" },
    { name: "Kathy Jennings", office: "Attorney General of Delaware", party: "Democrat", website: "https://attorneygeneral.delaware.gov", contact: "https://attorneygeneral.delaware.gov/contact/", q: "Kathy Jennings Delaware Attorney General" },
    { name: "Charuni Patibanda-Sanchez", office: "Secretary of State of Delaware", party: "Democrat", website: "https://sos.delaware.gov", contact: "https://sos.delaware.gov/contact/", q: "Charuni Patibanda-Sanchez Delaware Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/de/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Delaware State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear DE rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear DE representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert DE representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear DE district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert DE district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Delaware: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
