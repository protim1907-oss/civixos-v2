...
const STATE = "Florida";
const STATE_ABBR = "FL";
// Florida districts are stored zero-padded (FL-01 .. FL-28).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Florida ${ordinal(num)} Congressional District`,
  ...
  // Florida elects Governor, Lt. Governor, and Attorney General statewide.
  // Verified current 2026-08-05: DeSantis (R); Lt. Gov Jay Collins (R, appointed
  // 2025 after Nuñez resigned); AG James Uthmeier (R, appointed 2025 after Moody
  // went to the U.S. Senate).
  const execDefs = [
    { name: "Ron DeSantis", office: "Governor of Florida", party: "Republican", website: "https://www.flgov.com", contact: "https://www.flgov.com/contact-us/", q: "Ron DeSantis Governor Florida" },
    { name: "Jay Collins", office: "Lieutenant Governor of Florida", party: "Republican", website: "https://www.flgov.com/lieutenant-governor", contact: "https://www.flgov.com/contact-us/", q: "Jay Collins Lieutenant Governor Florida" },
    { name: "James Uthmeier", office: "Attorney General of Florida", party: "Republican", website: "https://www.myfloridalegal.com", contact: "https://www.myfloridalegal.com/contact-us", q: "James Uthmeier Florida Attorney General" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/fl/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Florida State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear FL rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear FL representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert FL representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear FL district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert FL district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Florida: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
