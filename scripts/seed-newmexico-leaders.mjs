...
const STATE_ABBR = "NM";
// New Mexico districts are stored zero-padded (NM-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `New Mexico ${ordinal(num)} Congressional District`,
  ...
  const execDefs = [
    { name: "Michelle Lujan Grisham", office: "Governor of New Mexico", party: "Democrat", website: "https://www.governor.state.nm.us", contact: "https://www.governor.state.nm.us/contact-the-governor/", q: "Michelle Lujan Grisham Governor New Mexico" },
    { name: "Howie Morales", office: "Lieutenant Governor of New Mexico", party: "Democrat", website: "https://www.ltgov.state.nm.us", contact: "https://www.ltgov.state.nm.us/contact/", q: "Howie Morales Lieutenant Governor New Mexico" },
    { name: "Raúl Torrez", office: "Attorney General of New Mexico", party: "Democrat", website: "https://nmag.gov", contact: "https://nmag.gov/contact-us/", q: "Raúl Torrez New Mexico Attorney General" },
    { name: "Maggie Toulouse Oliver", office: "Secretary of State of New Mexico", party: "Democrat", website: "https://www.sos.nm.gov", contact: "https://www.sos.nm.gov/contact-us/", q: "Maggie Toulouse Oliver New Mexico Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/nm/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `New Mexico State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear NM rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear NM representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert NM representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear NM district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert NM district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded New Mexico: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
