...
const STATE_ABBR = "MN";
// Minnesota districts are stored zero-padded (MN-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Minnesota ${ordinal(num)} Congressional District`,
  ...
  // Gov Tim Walz, Lt. Gov Peggy Flanagan (still serving), AG Keith Ellison (all D).
  const execDefs = [
    { name: "Tim Walz", office: "Governor of Minnesota", party: "Democrat", website: "https://mn.gov/governor", contact: "https://mn.gov/governor/contact/", q: "Tim Walz Governor Minnesota" },
    { name: "Peggy Flanagan", office: "Lieutenant Governor of Minnesota", party: "Democrat", website: "https://mn.gov/governor/about/lt-governor/", contact: "https://mn.gov/governor/contact/", q: "Peggy Flanagan Lieutenant Governor Minnesota" },
    { name: "Keith Ellison", office: "Attorney General of Minnesota", party: "Democrat", website: "https://www.ag.state.mn.us", contact: "https://www.ag.state.mn.us/office/contact.asp", q: "Keith Ellison Minnesota Attorney General" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/mn/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Minnesota State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear MN rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear MN representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert MN representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear MN district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert MN district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Minnesota: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
