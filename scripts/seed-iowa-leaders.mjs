...
const STATE = "Iowa";
const STATE_ABBR = "IA";
// Iowa districts are stored zero-padded (IA-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Iowa ${ordinal(num)} Congressional District`,
  ...
  const execDefs = [
    { name: "Kim Reynolds", office: "Governor of Iowa", party: "Republican", website: "https://governor.iowa.gov", contact: "https://governor.iowa.gov/contact", q: "Kim Reynolds Governor Iowa" },
    { name: "Chris Cournoyer", office: "Lieutenant Governor of Iowa", party: "Republican", website: "https://governor.iowa.gov", contact: "https://governor.iowa.gov/contact", q: "Chris Cournoyer Lieutenant Governor Iowa" },
    { name: "Brenna Bird", office: "Attorney General of Iowa", party: "Republican", website: "https://www.iowaattorneygeneral.gov", contact: "https://www.iowaattorneygeneral.gov/contact-us", q: "Brenna Bird Iowa Attorney General" },
    { name: "Paul Pate", office: "Secretary of State of Iowa", party: "Republican", website: "https://sos.iowa.gov", contact: "https://sos.iowa.gov/about/contactus.html", q: "Paul Pate Iowa Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/ia/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Iowa State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear IA rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear IA representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert IA representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear IA district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert IA district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Iowa: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
