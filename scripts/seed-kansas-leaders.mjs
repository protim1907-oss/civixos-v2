...
const STATE = "Kansas";
const STATE_ABBR = "KS";
// Kansas districts are stored zero-padded (KS-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `Kansas ${ordinal(num)} Congressional District`,
  ...
  const execDefs = [
    { name: "Laura Kelly", office: "Governor of Kansas", party: "Democrat", website: "https://governor.kansas.gov", contact: "https://governor.kansas.gov/constituent-services/", q: "Laura Kelly Governor Kansas" },
    { name: "David Toland", office: "Lieutenant Governor of Kansas", party: "Democrat", website: "https://www.kansascommerce.gov", contact: "https://governor.kansas.gov/constituent-services/", q: "David Toland Lieutenant Governor Kansas" },
    { name: "Kris Kobach", office: "Attorney General of Kansas", party: "Republican", website: "https://ag.ks.gov", contact: "https://ag.ks.gov/about-the-office/contact-us", q: "Kris Kobach Kansas Attorney General" },
    { name: "Scott Schwab", office: "Secretary of State of Kansas", party: "Republican", website: "https://sos.ks.gov", contact: "https://sos.ks.gov/general-services/contact-us.html", q: "Scott Schwab Kansas Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/ks/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `Kansas State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear KS rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear KS representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert KS representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear KS district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert KS district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded Kansas: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
