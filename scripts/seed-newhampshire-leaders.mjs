...
const STATE_ABBR = "NH";
// New Hampshire districts are stored zero-padded (NH-01 ..).
...
async function main() {
  ...
  const houseDelegation = stFederal
    ...
    .map((p) => {
      ...
      return {
        ...
        office_label: `New Hampshire ${ordinal(num)} Congressional District`,
  ...
  const usSenators = stFederal
    ...
    .map((p) => {
      ...
      return representative({
        name: p.name.official_full,
        office: "U.S. Senator, New Hampshire",
        level: "senate",
        website: t.url,
  ...
  const execDefs = [
    { name: "Kelly Ayotte", office: "Governor of New Hampshire", party: "Republican", website: "https://www.governor.nh.gov", contact: "https://www.governor.nh.gov/contact-us", q: "Kelly Ayotte Governor New Hampshire" },
    { name: "John Formella", office: "Attorney General of New Hampshire", party: "Republican", website: "https://www.doj.nh.gov", contact: "https://www.doj.nh.gov/contact-us", q: "John Formella New Hampshire Attorney General" },
    { name: "David Scanlan", office: "Secretary of State of New Hampshire", party: "Republican", website: "https://www.sos.nh.gov", contact: "https://www.sos.nh.gov/contact", q: "David Scanlan New Hampshire Secretary of State" },
  ];
  ...
  const files = (
    await getJSON(
      "https://api.github.com/repos/openstates/people/contents/data/nh/legislature"
    )
  ...
  for (const doc of docs) {
    ...
    stateSenators.push(
      representative({
        name: doc.name,
        office: `New Hampshire State Senator, District ${num}`,
        level: "State Senate",
        website: homepage || "",
        contact: homepage || "",
  ...
  // ---- Write to Supabase (idempotent: clear NH rows, then insert) --------
  ...
  const { error: repDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (repDeleteError) {
    console.error("Failed to clear NH representatives:", repDeleteError);
    process.exit(1);
  }
  ...
  if (repInsertError) {
    console.error("Failed to insert NH representatives:", repInsertError);
    process.exit(1);
  }
  ...
  const { error: houseDeleteError } = await supabase
    ...
    .eq("state", STATE);
  if (houseDeleteError) {
    console.error("Failed to clear NH district representatives:", houseDeleteError);
    process.exit(1);
  }
  ...
  if (houseInsertError) {
    console.error("Failed to insert NH district representatives:", houseInsertError);
    process.exit(1);
  }

  const missingPhotos = representativesRows.filter((r) => !r.photo).map((r) => r.name);
  console.log(
    `Seeded New Hampshire: ${usSenators.length} U.S. senators, ${execs.length} statewide execs, ` +
