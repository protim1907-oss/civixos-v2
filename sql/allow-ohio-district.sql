-- Allow Ohio (OH) districts through the profiles registration gate.
--
...
-- states. This adds OH to the allowed set alongside TX, CA, IL, MD, CO, and NV.
--
...
BEGIN
  ...
  -- District codes look like "TX-35", "IL-10", "MD-1", "CO-1", "NV-1", "OH-11";
  -- take the state prefix.
  prefix := upper(split_part(btrim(NEW.district), '-', 1));

  IF prefix IN ('TX', 'CA', 'IL', 'MD', 'CO', 'NV', 'OH') THEN
    RETURN NEW;
  END IF;

  RAISE EXCEPTION
    'Civix250 registration is limited to Texas, California, Illinois, Maryland, Colorado, Nevada, and Ohio districts (got %)',
    NEW.district
