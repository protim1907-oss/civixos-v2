-- Allow North Carolina (NC) districts through the profiles registration gate.
--
...
-- states. This adds NC to the allowed set alongside TX, CA, IL, MD, CO, NV, OH,
-- GA, MI, NY, and VA.
--
...
BEGIN
  ...
  -- District codes look like "TX-35", "MD-01", "NC-01"; take the state prefix.
  prefix := upper(split_part(btrim(NEW.district), '-', 1));

  IF prefix IN ('TX', 'CA', 'IL', 'MD', 'CO', 'NV', 'OH', 'GA', 'MI', 'NY', 'VA', 'NC') THEN
    RETURN NEW;
  END IF;

  RAISE EXCEPTION
    'Civix250 registration is limited to Texas, California, Illinois, Maryland, Colorado, Nevada, Ohio, Georgia, Michigan, New York, Virginia, and North Carolina districts (got %)',
    NEW.district
