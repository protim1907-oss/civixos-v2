-- Allow Virginia (VA) districts through the profiles registration gate.
--
...
-- states. This adds VA to the allowed set alongside TX, CA, IL, MD, CO, NV, OH,
-- GA, MI, and NY.
--
...
BEGIN
  ...
  -- District codes look like "TX-35", "MD-01", "VA-01"; take the state prefix.
  prefix := upper(split_part(btrim(NEW.district), '-', 1));

  IF prefix IN ('TX', 'CA', 'IL', 'MD', 'CO', 'NV', 'OH', 'GA', 'MI', 'NY', 'VA') THEN
    RETURN NEW;
  END IF;

  RAISE EXCEPTION
    'Civix250 registration is limited to Texas, California, Illinois, Maryland, Colorado, Nevada, Ohio, Georgia, Michigan, New York, and Virginia districts (got %)',
    NEW.district
