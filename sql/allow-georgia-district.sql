-- Allow Georgia (GA) districts through the profiles registration gate.
--
...
-- states. This adds GA to the allowed set alongside TX, CA, IL, MD, CO, NV, OH.
--
...
BEGIN
  ...
  -- District codes look like "TX-35", "MD-01", "GA-01"; take the state prefix.
  prefix := upper(split_part(btrim(NEW.district), '-', 1));

  IF prefix IN ('TX', 'CA', 'IL', 'MD', 'CO', 'NV', 'OH', 'GA') THEN
    RETURN NEW;
  END IF;

  RAISE EXCEPTION
    'Civix250 registration is limited to Texas, California, Illinois, Maryland, Colorado, Nevada, Ohio, and Georgia districts (got %)',
    NEW.district
