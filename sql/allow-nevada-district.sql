-- Allow Nevada (NV) districts through the profiles registration gate.
--
...
-- states. This adds NV to the allowed set alongside TX, CA, IL, MD, and CO.
--
...
BEGIN
  ...
  -- District codes look like "TX-35", "IL-10", "MD-1", "CO-1", "NV-1"; take the
  -- state prefix.
  prefix := upper(split_part(btrim(NEW.district), '-', 1));

  IF prefix IN ('TX', 'CA', 'IL', 'MD', 'CO', 'NV') THEN
    RETURN NEW;
  END IF;

  RAISE EXCEPTION
    'Civix250 registration is limited to Texas, California, Illinois, Maryland, Colorado, and Nevada districts (got %)',
    NEW.district
