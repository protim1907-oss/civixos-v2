-- Allow New York (NY) districts through the profiles registration gate.
--
...
-- states. This adds NY to the allowed set alongside TX, CA, IL, MD, CO, NV, OH,
-- GA, and MI.
--
...
BEGIN
  ...
  -- District codes look like "TX-35", "MD-01", "NY-01"; take the state prefix.
  prefix := upper(split_part(btrim(NEW.district), '-', 1));

  IF prefix IN ('TX', 'CA', 'IL', 'MD', 'CO', 'NV', 'OH', 'GA', 'MI', 'NY') THEN
    RETURN NEW;
  END IF;

  RAISE EXCEPTION
    'Civix250 registration is limited to Texas, California, Illinois, Maryland, Colorado, Nevada, Ohio, Georgia, Michigan, and New York districts (got %)',
    NEW.district
